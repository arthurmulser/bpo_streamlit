...
def make_spiral_data(n_per_class=400, turns=1.5):
    # duas espirais entrelacadas (mesmos dados do caso 5);
    rng = np.random.default_rng(kRANDOM_SEED)
    n = n_per_class
...
def run_arquitetura(nome, arch, x_train, x_test, y_train, y_test, n_epochs=100):
    ...
    model = MlpEspiralCamadaUnica(hidden_sizes=arch, n_epochs=n_epochs)
...
def main():
    ...
    model_8, loss_8, train_acc_8, test_acc_8 = run_arquitetura(
        "1 camada oculta", (8,), x_train, x_test, y_train, y_test, n_epochs=100)

    print("\n" + "=" * 64)
    print("  OBSERVACOES - 1 CAMADA OCULTA NA ESPIRAL")
    print("=" * 64)
    print("  - 1 camada x 3 neuronios: aprende a espiral quase por completo apos")
    print(f"    100 epocas (treino {1 - train_acc_3[-1]:.2%} / teste {1 - test_acc_3[-1]:.2%});")
    print("  - 1 camada x 8 neuronios: resultado final muito parecido")
    print(f"    (treino {1 - train_acc_8[-1]:.2%} / teste {1 - test_acc_8[-1]:.2%});")
    print("  - comparacao com o CASO 5 (mesmos dados, 100 epocas):")
    print("    * 1 x 3  -> teste ~2.08%;")
    print("    * 1 x 8  -> teste ~2.08%;")
    print("    * 3 x 4  -> teste ~2.08% (treino 0.54%);")
    print("    * 3 x 16 -> teste ~0.83% (treino 0.54%);")
    print("  - conclusao: essa espiral (1.5 voltas) e aprendivel ate por 1 camada")
    print("    oculta com poucos neuronios; a PROFUNDIDADE melhora o ajuste no")
    print("    treino (0.54% vs ~1.1%) e a largura maior (3x16) melhora a")
    print("    generalizacao (0.83% vs ~2.08%), mas a diferenca pratica e")
    print("    pequena; o limite do erro de teste (~2%) vem do ruido dos dados;")

    # grafico: fronteiras e curvas de erro;
    fig, axes = plt.subplots(2, 3, figsize=(15, 10))

    plot_decision_boundary(axes[0, 0], model_3, x_test, y_test,
                           "Espiral - 1 camada x 3 neuronios")
    plot_decision_boundary(axes[0, 1], model_8, x_test, y_test,
                           "Espiral - 1 camada x 8 neuronios")
    ...
    axes[1, 0].plot(loss_3, label='1x3', marker='o', markersize=2)
    axes[1, 0].plot(loss_8, label='1x8', marker='o', markersize=2)
    ...
    axes[1, 1].plot(train_acc_3, label='treino (1x3)', marker='o', markersize=2)
    axes[1, 1].plot(test_acc_3, label='teste (1x3)', marker='o', markersize=2)
    axes[1, 1].plot(train_acc_8, label='treino (1x8)', marker='o', markersize=2)
    axes[1, 1].plot(test_acc_8, label='teste (1x8)', marker='o', markersize=2)
    ...
    plt.tight_layout()
    output_path = Path(__file__).parent / "resultados_espiral_1camada.png"
