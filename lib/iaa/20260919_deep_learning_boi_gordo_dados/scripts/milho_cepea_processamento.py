"""
agrega semanalmente a serie diaria do milho cepea (a vista);
...
- saida: processed/milho_cepea_processed.csv;
"""
...
kARQUIVO_RAW = kRAIZ_DADOS / "raw" / "milho_cepea_raw.xls"
kPROCESSED = kRAIZ_DADOS / "processed"

kSAIDA = kPROCESSED / "milho_cepea_processed.csv"
kMETADATA = kPROCESSED / "milho_cepea_metadata.csv"
...
def carregar_ultima_serie_raw() -> pd.DataFrame:
    ...
    if not arquivo.exists():
        sys.exit("arquivo raw nao encontrado: raw/milho_cepea_raw.xls")
    ...
    df = pd.DataFrame(registros, columns=["data", "preco_milho_avista"])
    df["data"] = pd.to_datetime(df["data"], format="%d/%m/%Y")
    df["preco_milho_avista"] = pd.to_numeric(df["preco_milho_avista"], errors="coerce")
    df = df.dropna(subset=["preco_milho_avista"]).sort_values("data")
...
def agregar_semanal(df_diario: pd.DataFrame) -> pd.DataFrame:
    ...
    semanal = pd.DataFrame(
        {
            ...
            "preco_milho_avista_fechamento": grupos["preco_milho_avista"].last(),
            "preco_milho_avista_medio": grupos["preco_milho_avista"].mean(),
            "n_obs": grupos["preco_milho_avista"].count(),
        }
    ).reset_index()
    ...
    ordem = [
        ...
        "data_semana_inicio",
        "data_semana_fim",
        "preco_milho_avista_fechamento",
        "preco_milho_avista_medio",
        "n_obs",
        "ano_iso",
        "semana_iso",
    ]
    semanal["preco_milho_avista_fechamento"] = semanal["preco_milho_avista_fechamento"].round(2)
    semanal["preco_milho_avista_medio"] = semanal["preco_milho_avista_medio"].round(4)
...
def registrar_metadados(arquivo: Path, df: pd.DataFrame, arquivo_fonte: Path) -> None:
    registro = {
        ...
        "fonte": "CEPEA/ESALQ - Indicador do Milho (a vista)",
